...
namespace ConsoleUI.WithGenerics
{
    ...
    {
        // where T : is a class, ande has a empty contructor (new()) -> this is a limiter 
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class, new()
        {
            ...
            // Reflection (don't use it verry often) 
            // Reflection is very expensive (for example in case of speed)
            ...
            // Splits the header into one column reader per entry
            ...
            foreach (var row in lines)
            {
                ...
                //Splits the row into individual colums. Now the index
                //of this row matches the indey of the header so the
                //FirstName colmn header lines up with the FirstName
                //value in this row
                var vals = row.Split(';');

                //Loops through each header entry so we can compare that 
                //against the list of columns from reflection. Once we get
                //the matching column, we can do the "SetValue" methode to
                //set the column value foe our entry variable to the vals
        ...
        public static void SaveToTextFile<T>(List<T> data, string filePath) where T : class, new()
        {
            ...
            // Add the column header entries to the first line 
